              ...
              <div className="flex flex-col md:flex-row gap-4 p-2">
                <div className="w-full md:w-1/4 space-y-2 shrink-0">
                  {notesContent.map((item, idx) => (
                    <button key={idx} onClick={() => { setPageIndex(idx); if (contentRef.current) contentRef.current.scrollTop = 0; }} 
                      className={`w-full text-sm text-left p-4 rounded border transition-colors truncate ${pageIndex === idx ? "bg-emerald-600 border-emerald-500 text-white" : "bg-transparent border-slate-700 text-slate-300 hover:border-emerald-500"}`}>
                      {item.title}
                    </button>
                  ))}
                </div>
                <div ref={contentRef} className="w-full md:w-3/4 pl-0 md:pl-2 overflow-y-auto max-h-[80vh]">
