...
const GlobalStyles = () => (
  <style>{`
    @keyframes scan { 0% { background-position: 0 0; } 100% { background-position: 50px 50px; } }
...
export default function App() {
  ...
  return (
    <div className="bg-[#030712] text-slate-100 font-mono min-h-screen flex flex-col relative overflow-hidden">
      ...
      {/* ANIMATED BACKGROUND */}
      <div className="fixed inset-0 z-[-1] opacity-30 pointer-events-none animate-scan bg-[linear-gradient(rgba(16,185,129,0.2)_1px,transparent_1px),linear-gradient(90deg,rgba(16,185,129,0.2)_1px,transparent_1px)] bg-[size:50px_50px]"></div>
      ...
      {activeView && (
        <div className="fixed inset-0 z-[100] bg-black/95 p-12 overflow-y-auto animate-in fade-in zoom-in duration-300">
          ...
          <div className="max-w-6xl mx-auto text-white">
            ...
            {activeView === 'jobs' && <div className="max-w-4xl mx-auto"><h1 className="text-4xl font-black mb-8">ACTIVE_OPENINGS</h1><div className="bg-slate-900 rounded border border-slate-800">{jobOpenings.map((job, idx) => <div key={idx} className="flex items-center justify-between p-6 border-b border-slate-800"><div><p className="text-emerald-400 font-bold text-xs">{job.company}</p><h2 className="text-lg font-semibold">{job.role}</h2></div><a href={job.link} className="px-6 py-2 bg-indigo-600 rounded text-sm hover:bg-indigo-500">APPLY</a></div>)}</div></div>}
            {activeView === 'referralForm' && <div className="max-w-2xl mx-auto text-center"><h1 className="text-3xl mb-8">SUBMIT_REFERRAL_DATA</h1></div>}
            {activeView === 'availability' && <div className="max-w-4xl mx-auto text-center"><h1 className="text-3xl">REFERRAL_SLOTS_AVAILABLE</h1></div>}
          </div>
        </div>
      )}
