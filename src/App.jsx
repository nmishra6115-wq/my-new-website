...
export default function App() {
  ...
  return (
    <div className="text-slate-100 font-mono min-h-screen flex flex-col bg-[#030712]">
      ...
      {/* 2. MAIN CONTENT (Video + Feed + Cards + News) */}
      {!activeView ? (
        <main className="flex-grow w-full">
          <section className="w-full relative bg-black"><video className="w-full h-[500px] object-cover" autoPlay muted loop playsInline><source src="/intro.mp4" type="video/mp4" /></video></section>
        ...
        <div className="p-12 text-center flex-grow"><button onClick={() => setActiveView(null)} className="text-emerald-500 mb-8">&larr; BACK</button><h1 className="text-4xl font-black uppercase text-emerald-500">{activeView}</h1></div>
      )}

      {/* 3. FOOTER */}
      <footer className="py-10 text-center text-slate-500 border-t border-white/5 uppercase text-xs">© 2026 AML_DECODE / Designed by @Nitesh</footer>
    </div>
  );
}