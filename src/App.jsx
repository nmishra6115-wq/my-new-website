    ...
    <div className="text-slate-100 font-mono min-h-screen flex flex-col relative bg-[#030712]">
      ...
      <div className="w-full bg-emerald-950/20 border-y border-emerald-500/20 py-3 overflow-hidden">
        <div className="animate-slow-scroll whitespace-nowrap">
          <span className="text-emerald-500 font-black tracking-[0.2em] uppercase text-sm">● ANNOUNCEMENTS: I am still upgrading the website to few things work, it will be fully functionl by 05/28/2026.</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 p-12 overflow-y-auto animate-in fade-in zoom-in duration-300">
          ...
          <div className="max-w-4xl mx-auto text-white">
            ...
            {activeView === 'referralForm' && (
              <div className="max-w-xl mx-auto"><h1 className="text-3xl font-black mb-8 uppercase text-emerald-500">Submit Referral</h1>
              <form className="space-y-4" onSubmit={async (e) => { 
                ...
                <textarea placeholder="Description" className="w-full p-4 bg-[#030712] border border-slate-700 rounded" required></textarea>
                <button type="submit" className="w-full py-4 bg-emerald-600 font-bold uppercase">Send Data</button>
              </form></div>
            )}
            
            {activeView === 'available' && (
              <div className="max-w-4xl mx-auto"><h1 className="text-3xl font-black mb-6 uppercase tracking-widest text-emerald-500">Live Availability</h1>
              {submissions.map((s, i) => <div key={i} className="p-4 mb-4 border border-slate-700 bg-slate-900 rounded"><p className="font-bold">{s.name} - {s.role} at {s.company}</p></div>)}
              <div className="mt-8 text-center"><a href="mailto:nitesh@example.com?subject=Interested in Referral" className="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-black font-black uppercase tracking-widest transition-all">I am interested</a></div></div>
            )}
